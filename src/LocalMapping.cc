...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 处理新的关键帧，计算其词袋描述子并插入地图中
            // 首先将新的关键帧Ki作为新的节点Ki加入Covibility Graph，
            // 并且更新与那些能够共享地图点的关键帧节点相连接的边。
            // 同时更新关键帧Ki的生长树，并计算表示关键帧的词袋BOW。
            ProcessNewKeyFrame();

            // Check recent MapPoints，剔除冗余点
            MapPointCulling();

            // Triangulate new MapPoints
            // 三角化计算新的地图点
            // 通过将检测到的ORB特征点，找到Covisibility Graph中与之相连的关键帧Kc，
            // 进行特征匹配，然后将匹配到的特征点进行三角化。
            // 对于没有匹配上的点，本文又与其他关键帧中未被匹配的特征点进行匹配。
            // 匹配方法使用的是之前的方法，并且将不满足对极几何约束的匹配点舍弃。
            // ORB特征点对三角化后，检查正向景深、视差、反投影误差和尺度一致性，这时才得到地图点。
            // 一个地图点是通过两个关键帧观察到的，而它也可以投影到与之相连的其他关键帧中，
            // 这个时候可以使用Tracking部分的跟踪局部地图来在附近的关键帧中找到匹配。得到更多的地图点。
            CreateNewMapPoints();
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // covisibility graph 每个关键帧处的相机位置之间的关系“图”
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    nMPs++;
                    // 为了保存地图点，必须在创建该点云的前3帧测试通过约束，才能真正被保存，
                    // 这样才能保证可跟踪且不容易在三角化时出现较大误差。
