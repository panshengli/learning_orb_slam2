...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mImGray.channels()==3)
    {
        if(mbRGB)
            ...
            cvtColor(mImGray,mImGray,CV_BGR2GRAY);//转为灰度图
    }
    else if(mImGray.channels()==4)
    {
        if(mbRGB)
    ...
    // 如果没有初始化或没有关键帧,使用的是单目相机，所以在跟踪时需要判断是否初始化或有没有前序关键帧
    // 如果没有，使用mpIniORBextractor的参数进行特征点检测
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                // 当前帧与最近邻关键帧的匹配也失败了，那么意味着需要重新定位才能继续跟踪
                bOK = Relocalization();
            }
        }
        else
...
bool Tracking::Relocalization()
{
    ...
    // 计算当前帧的BOW向量，在关键帧词典数据库中选取若干关键帧作为候选
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        else
        {
            ...
            {
                // 寻找有足够多的特征点匹配的关键帧；最后，利用RANSAC迭代，然后使用PnP算法求解位姿
